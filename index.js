...
client.on(Events.InteractionCreate, async interaction => {
    ...
    if (await hasPermission(guild.id, commandName, member)) {
        if (commandName === 'msg') {
            ...
            if (type === 'maintenance') {
                embed = new EmbedBuilder()
                    ...
                    .setDescription('Scheduled maintenance is ongoing.\n\n🔧 We are improving your experience. Please wait!')
                ...
                embed = new EmbedBuilder()
                    ...
                    .setDescription('Server is currently offline or stopped.\n\n⚠️ Wait for updates.')
        ...
        if (commandName === 'setup') {
            try {
                const channel = await guild.channels.create({
                    ...
                    reason: 'Server monitoring channel created by bot'
                });

                setupChannels.set(guild.id, channel.id);
                monitoringStatus.set(guild.id, true);

                await interaction.reply(`✅ Monitoring channel created: ${channel}`);
            } catch (error) {
        ...
        if (commandName === 'stop') {
            ...
            await interaction.reply('🛑 Server monitoring stopped.');
        }

        if (commandName === 'start') {
            if (setupChannels.has(guild.id)) {
                monitoringStatus.set(guild.id, true);
                await interaction.reply('▶️ Server monitoring resumed.');
            } else {
                await interaction.reply('⚠️ You must run `/setup` first.');
            }
        }
        ...
        if (commandName === 'perm') {
            ...
            await interaction.reply(`✅ ${toggle}ed ${permission} for ${role}`);
        }
        ...
        if (commandName === 'del') {
            ...
            await interaction.reply('🧹 Deleted last 100 messages.');
        }
